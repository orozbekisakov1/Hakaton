...
from .forms import UserRegisterForm, UserUpdateForm, ProfileUpdateForm, ProductForm, BuyerRegisterForm, BuyerUpdateForm
...
def buyer_profile(request):
    if request.method == 'POST':
        buyer_form = BuyerUpdateForm(request.POST, instance=request.user)
        profile_form = BuyerUpdateForm(
            request.POST,
            request.FILES,
            instance=request.user.buyer
        )
        if buyer_form.is_valid() and profile_form.is_valid():
            buyer_form.save()
            profile_form.save()
            messages.success(request, f'Updated')
            return redirect('bprofile')
    else:
        buyer_form = BuyerUpdateForm(instance=request.user)
        profile_form = ProfileUpdateForm(instance=request.user.profile)

    context = {
            'user_form': buyer_form,
            'profile_form': profile_form
        }
    return render(request, 'users/profile.html', context)
